...
python train.py --folder data --n-ranks 1 --device cpu \
  ...
  --model-file outputs/smoke/model.txt \
  --report-data outputs/smoke/report.npz
python plot_training_results.py \
  --report-data outputs/smoke/report.npz --plots-dir outputs/smoke/plots
```
...
python train.py --folder data --n-ranks 40 --device auto \
  ...
  --checkpoint outputs/full/model.pt \
  --model-file outputs/full/model.txt \
  --report-data outputs/full/report.npz
python plot_training_results.py \
  --report-data outputs/full/report.npz --plots-dir outputs/full/plots
```
...
python compare_training_cases.py \
  --raw-report outputs/scaling/report.npz \
  --preprocessed-report outputs/full/report.npz \
...
init_model("outputs/full/model.pt", device="cpu")
