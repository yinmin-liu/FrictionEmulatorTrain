...
static void SaveAlpha2WeertmanData(IssmDouble C, IssmDouble vmag, IssmDouble alpha2){/*{{{*/

	int rank = IssmComm::GetRank();

	std::ostringstream fname;
	fname << "/home/yinmin/totten_yinmin/Helheim/data_friction/weertman_rank_" << rank << ".csv";

	std::ofstream out(fname.str().c_str(), std::ios::app);

	if(!out.is_open()){
		_printf_("WARNING: could not open file " << fname.str() << " for writing\n");
		return;
	}

	/* CSV format: C2,vmag,alpha2 */
...
void Friction::GetAlpha2Weertman(IssmDouble* palpha2, Gauss* gauss){/*{{{*/
	...
	/* Save training data */
	// SaveAlpha2WeertmanData(C, vmag, alpha2);
